...
docker save $TAG | tar --extract --to-stdout --file=- --wildcards '*/layer.tar' | tar x

guix shell --container --development apache-arrow --preserve='^VERSION$' --no-cwd --share=.=/work -- bash <<-EOF
cd /work
. env.sh
mkdir build install
